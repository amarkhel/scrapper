...
object MafiaPairsApp {
  def pairsStat = {
      val games = DB().loadGames(year=2016, loc=Location.OZHA, minCount = 7)
      val filtered = games.flatMap(_.statistics.mafiaPairsNames)
      val grouped = filtered.groupBy(identity).map(g => g._1 -> g._2.size).filter(_._2 > 5)
      val sorted = grouped.map(x =>(x._1._1, x._1._2, x._2)).toSeq.sortBy(_._3).reverse.zipWithIndex

      println(s"[spoiler=Статистика самых частых напарников-мафов]")
      println(s"[table]")
      println(s"[tbody]")
  ...
  def pairsStatWin = {
      ...
      val filtered = games.flatMap(g => {
        val pairs = g.statistics.mafiaPairsNames
        val res = g.tournamentResult
        for (pair <- pairs) yield ((pair._1, pair._2), winPoints(res))
      })
      val grouped = filtered.groupBy(_._1).map(c => c._1 -> c._2.map(_._2)).filter(_._2.size > 4)
      val sorted = grouped.map(x =>(x._1._1, x._1._2, x._2.size, x._2.sum)).toSeq.sortBy(a => a._4.toDouble / a._3).reverse.zipWithIndex

      println(s"[spoiler=Статистика самых частых напарников-мафов процент побед]")
      println(s"[table]")
      println(s"[tbody]")
